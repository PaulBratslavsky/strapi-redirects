...
module.exports = ({ strapi }) => ({
   ...
   */
  findOne: async (id) => {
    const results = await strapi.documents('plugin::redirects.redirect').findOne({
      documentId: id
    });
  ...
  findAll: async (params = {}) => {
    ...
    const redirects = await strapi.documents('plugin::redirects.redirect').findMany({
      sort,
      start,
      limit,
      filters,
  ...
  create: async ({ body }) => {
    ...
    const results = await strapi.documents('plugin::redirects.redirect').create(body);
    return results;
  },
  ...
  update: async (id, { body }) => {
    ...
    const results = await strapi.documents('plugin::redirects.redirect').update({
      documentId: id,
      data: body
    });
   ...
   */
  delete: async (id) => {
    const results = await strapi.documents('plugin::redirects.redirect').delete({
      documentId: id
    });
    ...
    for (const row of body.data) {
      ...
      try {
        ...
        if (existingRedirects.length > 0) {
          ...
          operationResult = await strapi.documents('plugin::redirects.redirect').update({
            documentId: id,
            data: row
          });
          ...
          operationResult = await strapi.documents('plugin::redirects.redirect').create({ data: row });
