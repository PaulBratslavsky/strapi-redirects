...
async function queryRedirects(url, id, returnCount = false) {
  try {
    ...
    const results = (await strapi.documents('plugin::redirects.redirect').findMany({ filters })) || [];
...
async function validateRedirect(redirect, isUpdate = false) {
  ...
  if (!isUpdate) {
    ...
    const existingRedirect = await strapi.documents('plugin::redirects.redirect').findOne({
      documentId: "__TODO__"
    });
