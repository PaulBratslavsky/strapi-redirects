...
const RedirectDetailPage = () => {
      ...
      if (isNewRedirect) {
        ...
        if (redirect || redirect.id) {
          if (submitMore) {
            ...
            // history.push(`/plugins/${PLUGIN_ID}/${redirect.id}`);
          }
        }
      } else {
  ...
  const createRedirect = async (redirect) => {
    const result = await post(`/${PLUGIN_ID}`, { body: { data: redirect } });
  ...
  return <h1>fix the detail page</h1>

  return (
    <Box>
      <BaseHeaderLayout
        navigationAction={
          <Link startIcon={<ArrowLeftIcon />} to={`/plugins/${PLUGIN_ID}`}>
