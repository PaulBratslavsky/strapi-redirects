...
export const ImportModal = ({ onClose }) => {
    ...
    try {
      ...
      toggleNotification({
        ...
        message: formatMessage({
          id: "my.message.id", // is the id required?
  ...
  const uploadData = async () => {
    ...
    try {
      const { data } = await post("/redirects/import", {
        body: { data: redirects },
      });
      setRedirects(data);
      ...
      const notificationMessageId = allCreated
        ? "modal.import.success.message"
        : "modal.import.partial.message";
      ...
      toggleNotification({
        ...
        message: formatMessage({
          id: "my.message.id", // is the id required?
          defaultMessage: formatMessage({ id: getTrad(notificationMessageId) }),
        }),
      });
    } catch (err) {
  ...
  const showFileDragAndDrop =
    !uploadingData &&
    uploadState === ModalState.UNSET &&
    redirects.length === 0;

  // Show the table of redirects in both UNSET and PARTIAL states if there's data
  const showData =
    !uploadingData &&
    (uploadState === ModalState.UNSET || uploadState === ModalState.PARTIAL) &&
    redirects.length > 0;

  const showSuccess = !uploadingData && uploadState === ModalState.SUCCESS;
  const showNothingToImport =
    !uploadingData && uploadState === ModalState.NOTHING;

  // Buttons visibility
  const showPartialSuccess =
    !uploadingData && showData && uploadState === ModalState.UNSET;
  const showContinueButton =
    !uploadingData && showData && uploadState === ModalState.PARTIAL;

  return <h1>fix the modal</h1>
  return (
    <Portal>
            ...
            {showContinueButton ? (
              <>
                ...
                <Typography textColor="neutral800" as="p">
                  {formatMessage({
                    id: getTrad("modal.import.partial.description"),
                  })}
                </Typography>
              </>
            ) : (
